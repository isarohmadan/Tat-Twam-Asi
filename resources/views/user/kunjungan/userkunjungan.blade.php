        ...
        <tbody>
            @foreach($kunjungans as $kunjungan)
                <tr>
                    ...
                    <td>
                        <span class="badge bg-{{ $kunjungan->status == 'disetujui' ? 'success' : 
                                          ($kunjungan->status == 'ditolak' ? 'danger' : 'warning') }}">
                            {{ ucfirst($kunjungan->status) }}
                        </span>
                    </td>
                    <td>
                        @if($kunjungan->status == 'ditolak' && $kunjungan->alasan_penolakan)
                            <button type="button" class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#alasanModal{{ $kunjungan->id }}">
                                Lihat Alasan
                            </button>
                            
                            <!-- Modal Alasan Penolakan -->
                            <div class="modal fade" id="alasanModal{{ $kunjungan->id }}" tabindex="-1" aria-labelledby="alasanModalLabel" aria-hidden="true">
                        ...
                        @elseif($kunjungan->status == 'disetujui' && $kunjungan->catatan)
                            <button type="button" class="btn btn-sm btn-info" data-bs-toggle="modal" data-bs-target="#catatanModal{{ $kunjungan->id }}">
                                Lihat Catatan
                            </button>
                            
                            <!-- Modal Catatan Persetujuan -->
                            <div class="modal fade" id="catatanModal{{ $kunjungan->id }}" tabindex="-1" aria-labelledby="catatanModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                    ...
                    <td>
                        <button type="button" class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#detailModal{{ $kunjungan->id }}">
                            Detail
                        </button>
                        
                        <!-- Modal Detail Kunjungan -->
                        <div class="modal fade" id="detailModal{{ $kunjungan->id }}" tabindex="-1" aria-labelledby="detailModalLabel" aria-hidden="true">
